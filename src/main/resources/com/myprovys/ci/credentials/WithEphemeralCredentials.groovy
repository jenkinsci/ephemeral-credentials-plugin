...
/**
 * Backs the {@code withEphemeralCredentials} global step. Shipped as Groovy
 ...
 * uses for its Docker.groovy, and the reason calling script.lock(...) /
 * script.input(...) here is safe: this is genuine CPS-interpreted code, not
 * a plain Java call reaching into a stored script reference.
 *
 ...
 * {@code WorkflowRun} isn't Java-serializable. The actual {@code Run} is
 * re-resolved via {@code Run.fromExternalizableId(...)} right before each use
 * and never held across a {@code lock}/{@code input} pause point - same
 * reasoning applies to the {@code EphemeralCredentialsProvider} singleton,
 * fetched fresh rather than cached in a field.
 */
